...
Silverlight.createObject=function(l,g,m,j,k,i,h)
{
	...
	else 
	{
		...
		setTimeout("CheckSilverlightInstall('"+b.version+"')",100);
	}
...
//		a=a.replace("{4}",f+h);
	...
	else
	{
		...
		if(c=="Windows")
			...
			b="92811";
		a='<div style="width: 205px; height: 67px; background-color: #FFFFFF"><img onclick="Silverlight.followFWLink({0});" style="border:0; cursor:pointer" src="{1}" alt="Get Microsoft Silverlight"/></div>'
	}
	a=a.replace("{0}",b);
//	a=a.replace("{1}",f+d);
	a=a.replace("{1}","../icons/silverlight1.0.png");
	return a
};
