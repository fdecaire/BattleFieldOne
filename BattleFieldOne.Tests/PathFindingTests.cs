...
namespace BattleFieldOne.Tests
{
	...
	public class PathFindingTests
	{
		...
		[TestMethod]
		public void TestUnitPathThroughForest()
		{
			GameBoard gameBoard = new GameBoard();
			gameBoard.InitializeBoard(7, 6);
			...
			Assert.AreEqual(6, unit.Path.WayPoint.Count);
			Assert.AreEqual(new Point(1, 3), unit.Path.WayPoint[0]);
			Assert.AreEqual(new Point(2, 3), unit.Path.WayPoint[1]);
			Assert.AreEqual(new Point(3, 3), unit.Path.WayPoint[2]);
			Assert.AreEqual(new Point(4, 3), unit.Path.WayPoint[3]);
			Assert.AreEqual(new Point(5, 3), unit.Path.WayPoint[4]);
			Assert.AreEqual(new Point(6, 3), unit.Path.WayPoint[5]);
		}

		[TestMethod]
		public void TestTankPathAroundForest()
		{
			GameBoard gameBoard = new GameBoard();
			gameBoard.InitializeBoard(7, 6);
			...
			// add allied and german units
			UnitClass unit = new UnitClass(2, NATIONALITY.German, 0, 3, 1);
